...
int main(void)
{
	...
	if (p_list != NULL)
	{
		if ((p_elem = class_list_create_element(p_list, &six)) == NULL)
			printf("Test \"03_create.c\": ERROR1\n");
		if ((p_elem2 = class_list_create_element(p_list, &seven)) == NULL)
			printf("Test \"03_create.c\": ERROR1.2\n");
		if ((p_elem3 = class_list_create_element(p_list, &eight)) == NULL)
			printf("Test \"03_create.c\": ERROR1.3\n");
		if (*(int *)(p_elem->p_content) != 6)
			printf("Test \"03_create.c\": ERROR2\n");
		class_list_add_to_end(p_list, p_elem);
		if (p_list->length != 1)
			printf("Test \"03_add.c\": ERROR5  %lu \n", p_list->length);
		if (*(int *)(p_list->p_element->p_content) != 6)
			printf("Test \"03_add.c\": ERROR2.1\n");
		class_list_add_to_end(p_list, p_elem2);
		if (p_list->length != 2)
			printf("Test \"03_add.c\": ERROR5  %lu \n", p_list->length);
		if (*(int *)(p_list->p_element->p_content) != 6)
			printf("Test \"03_add.c\": ERROR2.2 %d \n", *(int *)(p_list->p_element->p_content));
		...
		if (p_list == NULL)
			...
			printf("Test \"03_construct.c\": ERROR3\n");
	}
	else
		printf("Test \"03_construct.c\": ERROR4\n");
	return (0);
}